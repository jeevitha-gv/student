...
if(!empty($_POST["classid"])) 
{
 ...
 $stmt = $dbh->prepare("SELECT rollno,name,id FROM student WHERE dept= :id order by rollno");
 $stmt->execute(array(':id' => $cid));
 ?><option value="">Select </option><?php
 while($row=$stmt->fetch(PDO::FETCH_ASSOC))
 {
  ?>
  <option value="<?php echo htmlentities($row['rollno']); ?>"><?php echo htmlentities($row['rollno']); ?>-<?php echo htmlentities($row['name']); ?></option>
  <?php
 }
}
